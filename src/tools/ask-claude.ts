...
export function createAskClaudeTool(toolContext: ToolContext): ConsultTool {
  return Object.freeze({
    ...
    inputSchema: {
      question: z.string().min(1).describe("The question or problem to analyze. Be specific."),
      context: z.string().optional().describe("Background: what you tried, constraints, relevant snippets."),
      ...commonToolShape
    },
    execute: async (rawArgs: Record<string, unknown>) => {
      const args = argsSchema.parse(rawArgs);
      const prompt = args.context === undefined ? args.question : `<background-context>\n${args.context}\n</background-context>\n\n${args.question}`;
      return toolContext.runClaude({ prompt, appendSystemPrompt: composeAdvisorPrompt(), addDirs: [], ...toRunnerBase(args) });
    }
  });
}
